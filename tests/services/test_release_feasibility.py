...
def test_missing_component_is_red_and_blocks_the_node_above(db_session):
    ...
    assert material_row["stock_qty"] == 5.0
...
def test_ignored_warehouse_stock_does_not_cover_demand(db_session):
    ...
    db_session.add(ItemWarehouseStock(item_id=int(material.item_id), warehouse_ref1c="wh-good", qty=4.0))
    db_session.add(ItemWarehouseStock(item_id=int(material.item_id), warehouse_ref1c="wh-bad", qty=96.0))
    db_session.flush()

    payload = analyze_release(db_session, product, 10.0)

    material_row = _by_article(payload, "ART-M1")
    assert material_row["stock_qty"] == 4.0
...
def test_root_stock_never_covers_the_release_task(db_session):
    ...
    assert payload["root"]["stock_qty"] == 1000.0
    material_row = _by_article(payload, "ART-M1")
