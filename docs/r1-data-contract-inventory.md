...
## 6. Открытые вопросы, не блокирующие R1

1. **Исторический отчёт:** нужно выбрать продуктовый режим «как было на дату»
   или «что система знала тогда» до R5/R10. До решения сохраняются обе даты:
   posting и accepted/correction, и не обещается реконструкция по одному
   текущему итогу.
