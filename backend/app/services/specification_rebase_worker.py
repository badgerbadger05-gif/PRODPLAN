...
def run_one_pending_specification_rebase(
    ...
    *,
    ...
    affected = _affected_runs(db, requests)
    if not affected:
        now = datetime.now(timezone.utc)
        for request in requests:
    ...
    run = affected[0]
    refs = _refs_for_run(db, run, requests)
    selected_requests = []
    for request in requests:
        spec = db.get(models.Specification, int(request.spec_id))
    ...
    remaining = _affected_runs(
        ...
        [
            ...
            *(
                ...
                if int(row.id) not in {int(current_row.id) for current_row in current}
            ),
        ],
    )
