...
| Чистый выпуск документа сборки (`max(sum(assembly_in) - sum(assembly_out), 0)` в пределах одного документа) | `backend/app/services/item_ledger/document_net_output.py`; читатели — реплей пополнения, аллокация выпуска плана и факты выпуска заказов на производство (`production_fact_projection.py`), собственного отбора фактов у них нет |
...
## Контракт устойчивой идентичности (R1)

Инвентарь полей, читателей и писателей хранится в
[`docs/r1-data-contract-inventory.md`](../docs/r1-data-contract-inventory.md).
`generation_id`, `parent_generation_id` и `snapshot_id` — границы публикации,
provenance и идемпотентности, но не бизнес-идентичности движения, плана,
резерва или заказа. `parent_generation_id` в текущей схеме является
проверенным ключом JSON `LedgerGeneration.source_watermarks`, а не отдельной
колонкой; обход sealed-цепочки разрешён только в `live_plan_scope.py`.

Полный физический ключ — `item + characteristic + organization + warehouse`;
для распределения резерва добавляется `planning_stock_pool`. Дробные нормы и
количества компонентов сохраняются через `Decimal(15,3)` без промежуточного
округления. Корневой выпуск и плитка барабана штучные. Равные даты разрешаются
полным детерминированным ключом периода, `run_id`, `requirement_id`, bucket и
устойчивого локального ID; произвольный `last/first` запрещён.

Факты, frozen-обязательства, текущие итоги, основания, технические метрики и
история бизнес-команд не смешиваются. У каждого класса один допустимый
писатель; GET, frontend, статус заказа и ad-hoc SQL не могут писать
предметный итог. Смена MRP работает с сохранённым остатком корней и отдельным
журналом successor, а не с техническим переносом старого поколения.

История имеет две обязательные даты: `posting_at` — когда движение произошло,
`known_at` — когда PRODPLAN принял факт или correction. Отчёты выбирают режим
явно: `as_occurred` либо `as_known`; молчаливого fallback между ними нет.
