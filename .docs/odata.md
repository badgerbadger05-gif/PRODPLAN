...
- Дочерние документы сохраняют штатную связь с родителем. Обычно это
  `ДокументОснование` и `ДокументОснование_Type`, но `Document_СборкаЗапасов`
  связывается с производственным заказом только через
  `ЗаказНаПроизводство_Key`: его составное основание не принимает
...
| Документ | Основание |
|---|---|
| `Document_ЗаказНаПроизводство` | из MRP — первичный, без основания; в цепочке «окраска↔сварка» окрасочный заказ создаётся первым без основания, сварочный — на основании окрасочного (сварная деталь входит в состав окрашенной): `ЗаказНаПроизводствоОснование_Key` (Edm.Guid, специализированное поле) + `ДокументОснование`/`_Type` |
| `Document_ПеремещениеЗапасов` | `StandardODATA.Document_ЗаказНаПроизводство` |
| `Document_СборкаЗапасов` | `ЗаказНаПроизводство_Key`; generic `ДокументОснование`/`_Type` не отправляются |
