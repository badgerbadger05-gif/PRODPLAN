...
## Обновление статуса — 2025-10-22

Статус: исправлены два дефекта модуля расчёта заказов (буфер/оптимальная партия без финального cap; фантомные детские потребности при покрытии родителя со склада). Изменения в коде выполнены и задокументированы ниже.

### Изменения (backend)

1) Перепроизводство из‑за буфера/optimal batch  
- Добавлен финальный cap после лот‑сайзинга: normalized_qty = min(normalized_qty, final_qty) в [python.def compute()](backend/app/services/order_quantity_calculator.py:48). Правка добавлена сразу после нормализации (диапазон ~89–90 строк файла).

2) Фантомные заказы на нижнем уровне (детские компоненты создавались, когда родитель покрывается складом)  
- В [python.def compute_planning_preview()](backend/app/services/planning_service.py:994) вставлен дополнительный шаг «6.5) Adjust child gross by parent coverage» непосредственно перед неттингом (ориентир: ~1150 строка).  
- Алгоритм шага 6.5:
  - Для каждого item и бакета рассчитывается покрытие валовой потребности остатками+WIP (FIFO по датам).
  - Для каждого родителя величина покрытия по бакету пропорционально уменьшает валовую потребность детей: coverage_parent × per_unit.
  - Если детская валовая потребность уходит ≤ 0 — бакет удаляется.
- Пеггинг остаётся аналитическим и строится после формирования заказов (без изменений).

### Проверочный сценарий (минимальный)

- MPS: корневое изделие = 1 шт в бакете D.  
- Остатки: Деталь1 ≥ потребности изделия; Деталь2 = 0.  
- Ожидание: net‑потребности на Деталь2 нет; заказ на Деталь2 не создаётся.

### Влияние и совместимость

- Схема БД без изменений.  
- Производительность: добавлен один линейный проход по бакетам/компонентам в превью; асимптотика линейная по количеству корзин и среднему числу компонентов.  
- Safety stock: как и раньше применяется к валовой потребности до неттинга; перенос применения на net‑этап — отдельная задача (при необходимости).

### Изменённые файлы

- [backend/app/services/order_quantity_calculator.py](backend/app/services/order_quantity_calculator.py:1) — [python.def compute()](backend/app/services/order_quantity_calculator.py:48)  
- [backend/app/services/planning_service.py](backend/app/services/planning_service.py:1) — [python.def compute_planning_preview()](backend/app/services/planning_service.py:994) (вставлен блок «6.5» перед неттингом)

### Критерии приёмки

- Количество производственных заказов не превышает min(чистая потребность горизонта; максимум по комплектующим) даже при наличии буфера/optimal batch.  
- При наличии достаточного запаса промежуточной детали (Деталь1) детские компоненты (например, Деталь2) не попадают в заказы, если их потребность возникала только из части, покрытой складом у родителя.
## Текущий статус
...
## 2025-10-22 — Изменения в системе фильтрации по датам

Статус: выполнено. Внесены комплексные изменения в систему фильтрации данных планирования.

### Основные изменения

#### 1. Замена фильтрации с `bucket_date` на `start_date`
- **Бэкенд**: Все эндпоинты фильтрации теперь используют `start_date` вместо `bucket_date`
  - Эндпоинт `/results/{run_id}/production` - фильтрация по дате начала заказа
  - Эндпоинт `/results/{run_id}/production/grouped` - группировка по дате начала
  - Эндпоинт `/results/{run_id}/production/export` - экспорт по дате начала
- **Фронтенд**: Компоненты фильтрации обновлены для работы с `start_date`

#### 2. Удаление функциональности `day_date`
- Полностью удалена функциональность "Задание на день"
- Удален эндпоинт `/results/{run_id}/production/agenda_day`
- Удалена функция `get_run_production_agenda_day` из бэкенда
- Фронтенд больше не использует параметр `day_date`

#### 3. Добавление календарей к полям `date_from` и `date_to`
- **Компонент ProductionFilters**: добавлены календари с русской локализацией
- **Локализация**: русские названия дней недели и месяцев
- **Формат**: маска "YYYY-MM-DD" для совместимости с API

#### 4. Агрегация дублирующихся строк и суммирование значений
- Улучшена логика агрегации данных в группированных представлениях
- Суммирование количеств и нормативов для дублирующихся позиций
- Оптимизация производительности за счет серверной агрегации

### Технические детали реализации

#### Измененные файлы бэкенда:
- `backend/app/services/planning_service.py` - замена фильтрации, удаление agenda_day
- `backend/app/routers/plan.py` - обновление эндпоинтов, удаление day_date параметров

#### Измененные файлы фронтенда:
- `frontend/src/components/mrp/ProductionFilters.vue` - добавление календарей, русская локализация
- `frontend/src/composables/useProduction.ts` - обновление логики фильтрации
- `frontend/src/pages/MRPResultPage.vue` - удаление функциональности day_date

#### Русская локализация календаря:
- Дни недели: "Воскресенье_Понедельник_Вторник_Среда_Четверг_Пятница_Суббота"
- Короткие дни: "Вс_Пн_Вт_Ср_Чт_Пт_Сб"
- Месяцы: "Январь_Февраль_Март_Апрель_Май_Июнь_Июль_Август_Сентябрь_Октябрь_Ноябрь_Декабрь"
- Первый день недели: понедельник

### Влияние на систему
- **Совместимость API**: изменения не ломают обратную совместимость, параметры переименованы
- **Производительность**: улучшена за счет серверной агрегации
- **Пользовательский опыт**: упрощена фильтрация, добавлены удобные календари

## 2025-10-17 — Отмена локальных изменений и синхронизация с репозиторием
