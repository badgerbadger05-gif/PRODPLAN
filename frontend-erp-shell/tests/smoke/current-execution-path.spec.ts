...
test('follows persisted plan → MRP → journal basis → assembly queue links', async ({ page }) => {
  ...
  await expect(page.getByTestId('assembly-queue-row-plan-line:502')).toHaveClass('activeRow')
  await expect(page.getByText('accepted:g77:assembly_queue')).toBeVisible()
})
