...
function buildResultQuery(params: { format?: string; date_from?: string; date_to?: string; limit?: number; offset?: number } = {}) {
  const search = new URLSearchParams()
...
export function getPlanningResultProduction(runId: number, params: { date_from?: string; date_to?: string; limit?: number; offset?: number } = {}) {
  return api<MrpPagedResponse<MrpProductionRow>>(`/v1/plan/results/${runId}/production?${buildResultQuery(params)}`)
}

export function getPlanningResultPurchases(runId: number, params: { date_from?: string; date_to?: string; limit?: number; offset?: number } = {}) {
  return api<MrpPagedResponse<MrpPurchaseRow>>(`/v1/plan/results/${runId}/purchases?${buildResultQuery(params)}`)
}

export function getPlanningResultRework(runId: number, params: { date_from?: string; date_to?: string; limit?: number; offset?: number } = {}) {
...
export function exportPlanningResultProduction(runId: number, params: { format: 'csv' | 'xlsx'; date_from?: string; date_to?: string }) {
...
export function exportPlanningResultPurchases(runId: number, params: { format: 'csv' | 'xlsx'; date_from?: string; date_to?: string }) {
  return api<{ data_base64?: string; filename?: string; content_type?: string }>(
    `/v1/plan/results/${runId}/purchases/export?${buildResultQuery(params)}`,
  )
}

export function exportPlanningResultRework(runId: number, params: { format: 'csv' | 'xlsx'; date_from?: string; date_to?: string }) {
  return api<{ data_base64?: string; filename?: string; content_type?: string }>(
