...
История движений, исправлений и бизнес-решений сохраняется. Журнал изменения назначения растёт только при реальном изменении основания. Технические task/debug-записи получают ограниченную политику хранения; конкретный срок фиксируется в R1/R11. Контрольные копии миграции имеют отдельное назначение и срок, не подменяют runtime-историю. Возможность отчёта «как было известно тогда» требует сохранения версий оснований/правил, а не обещания восстановить прошлое по одним сегодняшним итогам.
...
| Исторический отчёт: «что было» или «что знали тогда» | Явно различать дату движения и дату принятия/коррекции | R1, до сокращения аудита |
...
Следующая конкретная работа: **R1 — инвентарь зависимостей и контракт устойчивой идентичности**, затем **R2 — изолированная PostgreSQL и baseline**. Первая функция плана изменений остаётся подготовкой к R4; её наличие не позволяет перескочить через R1–R3.
