...
Порог свежести принятого поколения (§40) применяется к читателям, но не к
самой публикации. Публикация — физический refresh, обязательственный refresh,
приёмка поколения и восстановление текущих областей от указателя — является
единственным механизмом, который делает принятое поколение свежим; проверка её
собственных расчётов по возрасту того же указателя даёт взаимную блокировку:
после простоя дольше порога ни одна публикация не может завершиться, и снять
это состояние нечем. Внутри публикации снимается только проверка возраста.
Согласованность с точным указателем, требуемые capabilities и операторская
инвалидация действуют по-прежнему, а за пределами публикации порог остаётся.
