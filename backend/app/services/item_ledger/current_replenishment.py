...
def apply_current_replenishment(
    ...
    *,
    ...
    _require_complete_scope(fact_rows, reserve_rows, complete_scope)
    ...
    if state is not None:
        ...
        if int(state.ledger_generation_id) != int(generation.id):
            raise CurrentReplenishmentError("current source marker belongs to another generation")
    ...
    previous = tuple(_as_allocation(row) for row in allocations)
    try:
    ...
    entries = (
        ...
        .filter(models.ReservationEntry.ledger_generation_id == int(generation.id))
        .with_for_update()
        .all()
    )
    ...
    for insertion in plan.insertions:
        ...
        row = models.ReservationConsumptionAllocation(
            ...
            planning_stock_pool=_text(entry.planning_stock_pool) or "default",
            idempotency_key=(
