...
Сам возраст считается от последней успешной сверки с 1С, а не от последней
публикации (§57). Физический refresh, который прочитал 1С до нового cutoff,
сошёлся по остаткам и не нашёл семантической дельты, продлевает свежесть
текущего указателя до этого cutoff, не создавая поколения: проверка
сохраняется на самом указателе (поколение неизменяемо), только вперёд, только
при валидной сходимости остатков и только для того поколения, от которого она
считалась, — любое перемещение указателя делает её нечитаемой. Неуспешная или
несошедшаяся проверка свежесть не продлевает. Исключение ровно одно: преемник,
унаследовавший cutoff прежнего поколения указателя (обязательственный refresh —
те же физические факты), получает проверку вместе с указателем, а преемник с
новым cutoff — никогда; решает это единственный писатель указателя
`planning_truth.move_pointer_to_generation`.
